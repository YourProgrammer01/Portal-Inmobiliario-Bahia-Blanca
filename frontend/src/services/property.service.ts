...
// --- Auth ---
export const loginService = async (email: string, password: string, coords?: { lat: number; lon: number }): Promise<AuthResponse> => {
  const { data } = await api.post<AuthResponse>('/auth/login', { email, password, coords })
  return data
}
